/* Registry of chat-enhancement features (ported from the smarts-claude-patch
   skill's standalone webview/assets/*.js|.css pair-per-feature model into
   this extension's inline-injection convention — see behaviorInject.ts for
   how these are assembled and spliced into extension.js/webview/index.js|css).

   Each entry's `js`/`css` is the feature's IIFE source (registers via
   window.__ccOnChatDoc, exactly like the shared bootstrap in
   behaviorBootstrap.ts expects) and matching stylesheet.
   `label` is shown in the FeatureToggle gear panel and the settings
   description; order matches load/sweep order (a dependent, e.g.
 ...
 * @returns {{js: string, css: string}[]} the infrastructure js/css source pairs
 */
