...
// the extension host. Two ADDITIVE patches, applied together, make this
// possible without touching Claude Code's own dispatch logic or calling
...
// hook events. See that hook's own header comment for why: the DOM-watching
// webview approach this bridge used to also carry -- a ccNotify message type
// -- was retired because it depended on guessing "a reply just finished" from
// transient DOM state (a Stop button disappearing, a dialog element
// appearing), which is fragile against any Claude Code bundle change. The
// CLI's own hook events are the authoritative, DOM-independent signal.)
//
...
//    chat-enhancement scripts (which run in the SAME document) then reuse
//    window.__ccVsCodeApi.postMessage(...) instead of ever calling
//    acquireVsCodeApi() themselves.
...
//    multiple listeners coexist safely. Ours recognizes two message shapes,
//    {type:"ccOpenExternal", url} and {type:"ccOpenFile", path}, and
//    dispatches to <local-vscode-alias>.env.openExternal(...) /
//    a small ccup_openFile(p) helper (also injected) that resolves p against
//    every open workspace folder (Uri.joinPath) plus as a bare absolute path
//    (Uri.file), opening the FIRST candidate that actually exists via
//    vscode.workspace.fs.stat, with vscode.window.showTextDocument(uri,
//    {preview:false}) -- VS Code's own editor-group model already reveals/
//    focuses an EXISTING non-preview tab for that same URI instead of
//    opening a duplicate, so "reuse the tab if it's already open" falls out
//    of {preview:false} for free, no extra bookkeeping needed. Everything
//    else is ignored, and Claude Code's own listener/dispatch is never
//    touched.
//
...
// byte-for-byte. The host-side removal regex spans from the marker to the
// literal ",null,this.disposables);" close (non-greedy, [\s\S]*?) rather than
...
// needing the removal regex updated in lockstep. If the anchor text is ever
// gone (Claude Code refactors either call site), apply()/present() simply
// no-op and the feature falls back to being unavailable -- never a thrown
// error, never a corrupted file.
...
export function applyHostBridge(extensionJs: string): { out: string; changed: boolean } {
  ...
  // grows. This keeps removeHostBridge's regex correct for BOTH an
  // already-applied OLD-shape file (upgrading from a version that only
  // handled ccOpenExternal) and this NEW shape, without needing two
  // different removal patterns. Resolves p (relative OR absolute) against
  // every open workspace folder (Uri.joinPath) plus as a bare absolute path
  // (Uri.file), opening the FIRST candidate that actually exists
  // (vscode.workspace.fs.stat, works for local and remote filesystems) via
  // showTextDocument(uri, {preview:false}) -- a non-preview tab, so VS
  // Code's own editor-group model reveals/focuses an EXISTING tab for that
  // URI instead of opening a duplicate. Silently no-ops if nothing resolves
  // (matches this codebase's established "never throw on a miss" convention).
  const injected =
